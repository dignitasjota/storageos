...
- **Integración real de control de accesos**: ✅ **Acceso del inquilino por QR/PIN desde el portal/móvil** implementado — copia cifrada (AES-256-GCM) del secreto pin/qr en `access_credentials.secret_encrypted` (migración `20260618120000`) para poder volver a mostrárselo a su dueño (el hash no es reversible); endpoints `GET /portal/me/access` (lista pin/qr activas con el valor descifrado) + `POST /portal/me/access/:id/regenerate`; UI en `/portal/consume` con QR (`qrcode.react`) + PIN + botón regenerar. El inquilino presenta el código en el lector y el flujo device→`/access/verify`→abrir existente lo valida. e2e `portal-access` 3/3. ✅ **Provider HTTP genérico + apertura remota** implementado — `HttpLockProvider` (`LOCK_PROVIDER=http`): POST firmado HMAC-SHA256 (`X-StorageOS-Signature: t=,v1=`) a la `controlUrl` del device; funciona con cualquier controlador HTTP (Shelly/ESP32/Home Assistant/gateway). `access_devices.control_url` + `control_secret_encrypted` (AES-GCM, migración `20260618140000`), configurables en el panel de devices. Apertura remota staff `POST /access/devices/:id/open` (owner/manager) → registra en `access_logs` (`remote_open_by_staff`); botón "Abrir (remoto)" en `/access/devices`. Tests: unit `http-lock.provider` 4/4 + e2e `access` 10/10. **Pendiente** (menor): integraciones específicas de fabricante (PTI/Noke/Sensata) si un cliente las exige; el provider HTTP cubre el caso general.
- **Subidas de precio a clientes en cartera (ECRI)** ✅ **Implementado** (2026-06-22): `RentIncreasesModule` + tablas `rent_increases`/`rent_increase_items` (RLS). La palanca de ingresos nº1 del self-storage: revisa la cuota de los inquilinos activos con **preaviso por email** + **aplicación programada** (cron `rent-increases.apply`). `POST /rent-increases/preview` (afectados + precio nuevo % / € + **delta de MRR**), `POST /rent-increases` (programa + envía aviso), apply manual + cron (sube `priceMonthly` + evento `price_changed` → la factura recurrente sale al precio nuevo), cancel. Scope: active/ending + antigüedad mínima + local/tipo. Permiso `contracts:manage`. UI `/rent-increases` (lista + wizard con preview + detalle). e2e `rent-increases` 1/1. **Pendiente**: aplicar el pricing dinámico de `InsightsService`.
- **Recargos por mora (late fees)** ✅ **Implementado** (2026-06-22): activa el tipo `late_fee` del dunning. Config opt-in por tenant (`late_fee_enabled`/`type`/`value`/`grace_days`) en `/settings/billing`. A los N días de vencimiento el dunning emite una **factura separada** de recargo (IVA 0, indemnizatorio; % del vencido o € fijo), idempotente (índice único `invoices.late_fee_for_invoice_id`). Manual: `POST /invoices/:id/late-fee` (`invoices:manage`) + botón en la factura vencida. e2e `late-fees` 1/1.
- **Seguro / protección de contenido recurrente** ✅ **Implementado** (2026-06-22): `InsuranceModule` + tabla `insurance_plans` (RLS) + `contracts.insurance_plan_id`/`insurance_price` (snapshot). Ingreso recurrente de alto margen: el plan se asigna al contrato (en el alta vía `CreateContractSchema.insurancePlanId` o en la ficha vía `PUT /contracts/:id/insurance`) y la prima se factura como **una línea más en la factura mensual** (`billing-jobs`). CRUD `/insurance-plans` (permisos `insurance:read`/`insurance:manage`); IVA por plan (default 21%, se vende como servicio de protección, no seguro regulado). UI `/insurance-plans` + card en la ficha del contrato + selector en el wizard. e2e `insurance` 1/1. **Pendiente**: ofrecerlo en el move-in self-service público + IPS 8% si se requiere seguro regulado.
- **Move-out self-service + encuesta de salida** ✅ **Implementado** (2026-06-22): el inquilino solicita la baja desde el portal, cerrando el ciclo del local desatendido. `GET /portal/me/contracts` + `POST /portal/me/contracts/:id/request-move-out {endDate}` → valida propiedad + preaviso (`cancellationNoticeDays`) → contrato a `ending` con la fecha + evento → emite `contract_move_out_requested`. Listeners: notificación in-app al staff + **encuesta de salida NPS** (reusa Reviews). El staff hace el check-out en la fecha (no auto-finaliza). UI: card "Mis contratos" + "Solicitar baja" en `/portal/consume`. Sin migración (reusa campos del contrato). e2e `move-out` 2/2. **Pendiente**: win-back de bajas (campaña a ex-clientes) + auto-finalización opcional por cron.
...
- **Check-in / check-out con fotos del trastero** _(propuesta 2026-06-22)_: capturar el estado del trastero (fotos) en la entrada y la salida, como **evidencia para fianzas y disputas** (y soporte del flujo de impago→subasta). Fotos a MinIO vinculadas al contrato/evento.
- **Mantenimiento recurrente + rondas de seguridad** _(propuesta 2026-06-22)_: tareas programadas (limpieza, revisiones periódicas) + checklists de rondas. Hoy las `tasks` son puntuales; faltan recurrencia y plantillas de checklist.
- **Portal del inquilino — autoservicio ampliado** _(propuesta 2026-06-22)_: (a) **reportar incidencias** desde el portal (hoy los `incidents` son solo staff); (b) **solicitar cambio/upgrade de trastero**; (c) **notificaciones push (PWA)** de pago/acceso/vencimiento.
- ~~**App PWA del inquilino**~~ ✅ **Implementado** (v1): el portal del inquilino es instalable como PWA — `manifest.webmanifest` (start_url `/portal/login`, scope `/portal`, display standalone, iconos SVG `any`+`maskable`), service worker `public/sw.js` (app-shell offline con fallback `/offline.html`, cache-first de `/_next/static`, **nunca cachea `/api` ni respuestas autenticadas**), registro vía `<PwaRegister>` (solo prod) y botón "Instalar app" (`beforeinstallprompt`) en el portal. El manifest se engancha solo en `(public)/portal/layout.tsx` (no en staff/admin). Reutiliza lo ya hecho (facturas, pago, acceso QR/PIN). Pendiente opcional: iconos PNG (hoy SVG, instala en Android; iOS usa "Añadir a inicio") y cacheo offline del código de acceso. Las incidencias del inquilino siguen sin exponerse en el portal.
...
- **Remesas SEPA (fichero bancario)** ✅ **Implementado** (2026-06-22): `SepaModule` genera el **XML pain.008.001.02** (CORE, AEB 19.14) que el operador sube a su banco — sin comisiones de pasarela. Config del acreedor (IBAN cifrado) + mandatos por cliente (IBAN cifrado + referencia + firma + FRST/RCUR) + remesa (preview de facturas domiciliables → genera XML → descargar → confirmar cobro marca las facturas pagadas + mandatos FRST→RCUR). Validación IBAN mod-97. Permisos `invoices:manage`/`payments:charge`/`billing:configure`. UI: `/sepa-remittances` + acreedor en `/settings/billing` + mandato en la ficha del cliente. e2e `sepa-remittances` 2/2. **Pendiente** (futuro): esquema B2B, conciliación N43 para devoluciones (R-transactions).
- **GoCardless** para SEPA recurrente gestionado (alternativa a las remesas DIY; más barato que Stripe para domiciliación).
...
- **Conciliación bancaria** (norma 43): casar cobros con el extracto. _(Complementa la remesa SEPA: el N43 trae las devoluciones/R-transactions para marcar impagos.)_
- **Informes fiscales** más allá de Veri\*Factu: libro registro de IVA, soporte 303/347.
- **Export contable A3/Sage** _(propuesta 2026-06-22)_: además de Holded (ya hecho), exportar a A3 y Sage, muy usados por asesorías españolas.

### Crecimiento y CRM

- ~~**Landing pública con disponibilidad + "reservar ahora"** (SEO)~~ ✅ **Implementado** (v1, landing del tenant con sección por local): página SSR indexable `/s/[slug]` (`(public)/s/[slug]/page.tsx`, server component con `generateMetadata` SEO + OpenGraph + canonical + JSON-LD `SelfStorage` por local) que muestra la empresa + cada local (dirección, contacto) con sus tipos + precio "desde" + plazas disponibles y CTA **"Reservar ahora"** → flujo `/book/[slug]` existente. Endpoint público `GET /public/landing/:slug` (`LandingModule`, VERSION_NEUTRAL + @Public + throttle 60/min, exento del legacy-redirect como el widget). ✅ **Páginas SEO por local + sitemap/robots**: `Facility.publicSlug` (único por tenant, autogenerado del nombre con dedupe, migración `20260618170000` + backfill SQL); página por local `/s/[slug]/[facility]` (SSR + metadata + JSON-LD + breadcrumb), endpoints `GET /public/landing/:slug/:facilitySlug` y `GET /public/landing/sitemap`; `app/sitemap.ts` (lista tenants activos + locales, usa `NEXT_PUBLIC_SITE_URL`) y `app/robots.ts` (allow `/`,`/s/`; disallow panel/portal/api/widget/book). e2e `landing` 4/4. ✅ **Editar slug + imágenes de local desde el panel**: pestaña "Ajustes" en `/facilities/[id]` (gated `facilities:manage`) con edición del `publicSlug` (el backend ya lo aceptaba) + gestión de imágenes — `facilities.images String[]` (keys MinIO), `POST /facilities/:id/images/upload-url` (presigned PUT) + `PUT /facilities/:id/images` (set de la lista, valida que cada key pertenece al local) → se muestran en la landing pública. Bucket **público dedicado** `storageos-public` (anonymous download; `uploads`/`invoices`/`plans` siguen privados). e2e `facility-images` 2/2.
- ~~**Reviews / NPS** post-contratación automatizado~~ ✅ **Implementado**: `ReviewsModule` con tabla `reviews` (RLS) + token público. Staff solicita la valoración (`POST /reviews/request`) o la pide el **cron `reviews.auto-request`** N días tras firmar (opt-in por tenant: `reviews_auto_request` + `review_request_delay_days`, gated `WORKERS_ENABLED_IN_API`). El inquilino la rellena en `/review/[token]` (NPS 0-10 + estrellas 1-5 + comentario); `POST /public/reviews/:token` la guarda y emite `domain.review_submitted` (nuevo trigger de automations). Panel `/reviews` con NPS (= %promotores − %detractores), media de estrellas, tasa de respuesta + historial. Email por plantilla `review_request_email` (con fallback inline para tenants antiguos). e2e `reviews` 4/4. ✅ **Reseñas en Google Business Profile (2026-06-22)**: `tenants.google_review_url` (config en `/settings/tenant/reviews`); tras una valoración de promotor (NPS≥9), `submitByToken` devuelve el link de Google y la página pública muestra un CTA "Reseñar en Google" (sube el ranking local). Detractores reciben `null`. e2e `google-reviews` 1/1.
- **Campañas segmentadas** ✅ **Implementado** (v1, email): `CampaignsModule` + tabla `campaigns` (RLS). Segmenta **clientes** (con/sin contrato activo, morosos, por tag) o **leads** (por estado/origen); `POST /campaigns/preview` cuenta la audiencia, `POST /campaigns` crea (borrador), `POST /campaigns/:id/send` encola una `communications` por destinatario en el outbox (`source=campaign:<id>`, cuerpo Handlebars renderizado con la whitelist `manual`, `scheduledFor` opcional). Permisos `communications:read`/`communications:send`. UI: página `/campaigns` (lista + wizard con audiencia/filtros/cuerpo + "Calcular audiencia" + enviar). e2e `campaigns` 1/1. **Pendiente**: WhatsApp (requiere plantilla WABA aprobada) + métricas de apertura.
- **Promociones** ✅ **Implementado** (gestión + aplicación): `PromotionsModule` expone la tabla `promotions` (que existía sin CRUD): `GET/POST/PATCH/DELETE /promotions` (permisos `promotions:read`/`promotions:manage`) + `POST /promotions/validate` (previsualiza el descuento). **Aplicación en el alta de contrato**: `CreateContractSchema.promotionCode` → el backend valida (activa/vigente/usos) y fija `discountAmount` recurrente + `discountReason` + incrementa `usedCount` (atómico, en la transacción del contrato); solo `percentage`/`fixed` (los `free_months` se gestionan pero aún no se aplican en alta). UI: página `/promotions` (CRUD) + campo "Código promocional" en el wizard de contrato (con validación en vivo). e2e `promotions` 2/2. **Pendiente**: aplicar `free_months` en facturación.
- **Programa de referidos** ✅ **Implementado** (encima de promociones): `ReferralsModule` + tabla `referrals` (RLS) + `customers.referral_code` (único por tenant, autogenerado) + flags de tenant `referral_enabled`/`referral_reward_type`/`referral_reward_value` (opt-in). Cada inquilino comparte su código; el nuevo cliente lo introduce en el **alta de staff** (`CreateCustomerSchema.referralCode`) o en el **self-service** (`/book/[slug]`), creando un `referral` `pending` (best-effort, nunca bloquea el alta). Al firmar el **primer contrato del referido**, un listener de `domain.contract_signed` lo marca `converted` y **genera una promoción de un solo uso** (`REF-XXXX`, `maxUses=1`) para el referidor con la recompensa configurada. El inquilino ve su código + referidos + recompensas en el **portal** (`GET /portal/me/referrals`); staff ve la lista + stats (`/referrals`) y la config (`/settings/tenant/referrals`). e2e `referrals` 2/2.
- **Reserva online con pago** _(propuesta 2026-06-22)_: cobrar la primera mensualidad/depósito al reservar en `/book/[slug]` (el booking self-service existe; falta integrar el pago — Redsys/Stripe ya disponibles). Convierte la landing en captación que cobra sola.
- **Win-back de bajas** _(propuesta 2026-06-22)_: campaña automática a ex-clientes (contratos `ended`) reusando `CampaignsModule`. Empareja con el move-out + encuesta de salida ya hechos.
- **Tracking de campañas (UTM) en leads** _(propuesta 2026-06-22)_: capturar `utm_source/medium/campaign` en el widget/booking y reportar conversión por origen en analytics.
...
- **Asistente IA para staff** (resúmenes de cliente, redacción de comunicaciones, Q&A del negocio en lenguaje natural).
- **Pricing dinámico aplicado** _(propuesta 2026-06-22)_: `InsightsService` ya **calcula** sugerencias de precio por ocupación pero son read-only; cerrar el bucle aplicándolas (un click o por reglas/umbral) → yield management real que muta los `pricing_rules`/precios.

### Plataforma y robustez técnica

- **Multi-idioma EN/CA/FR** (i18n con next-intl ya montado; hoy solo `es-ES`).
- ~~**Permisos más finos**~~ ✅ **Implementado** (capa de permisos discretos): catálogo `Permission` (`recurso:acción`) + `ROLE_PERMISSIONS` (mapa rol→permisos en código) + helpers en `packages/shared/src/auth/permissions.ts`. Backend: decorador `@RequirePermission(...)` + `PermissionsGuard` (APP_GUARD); `/auth/me` devuelve los permisos efectivos del usuario. Primera regla más fina real: `invoices:refund` solo `owner` (aunque `manager` pueda emitir). **RBAC v2 completado** (2026-06): todos los controllers de tenant migrados a `@RequirePermission`; el `RolesGuard`/`@Roles` se retiró como código muerto (cadena de guards `Throttler → JwtAuth → Permissions`). Frontend: hook `useHasPermission` + componente `<Can permission="…">` que gatea los botones de acción de las páginas principales contra el permiso del backend (cosmético). Tests: unit `permissions.guard` 11/11 + e2e `permissions` 18/18.
- ~~**Roles personalizados por tenant (RBAC en BD)**~~ ✅ **Implementado** (v1, capa de roles custom): tabla `tenant_roles` `(tenant_id, name, description, permissions[], base_role)` con RLS + `users.tenant_role_id` (FK, ON DELETE SET NULL). Permisos efectivos = los del rol custom si está asignado, si no los del rol enum; se resuelven al emitir el access JWT (claim `permissions`) y en `/auth/me`. `PermissionsGuard` lee los permisos del token (fallback al mapa enum para tokens antiguos). CRUD `TenantRolesController` (`/settings/roles`, owner-only) + asignación `PATCH /settings/users/:id/tenant-role`. UI: página `/settings/roles` con matriz de permisos por recurso + selector de rol custom en el dialog de edición de usuario. Tests: e2e `tenant-roles` 3/3. (La migración v2 de todos los endpoints a `@RequirePermission` ya está hecha; el `base_role` se mantiene solo como semilla de permisos del rol custom.) (Pendiente: 2FA obligatorio configurable por más roles.)
...
- ~~**Playwright en CI bloqueante**~~ ✅ **Implementado y verde 5/5** (run `e2e.yml`): el gate era rojo permanente por varias causas que se han corregido — (1) `e2e.yml` no compilaba `@storageos/database` antes de `nest build` → ~709 errores TS en cascada (`Cannot find module '@storageos/database'`); (2) no sembraba el super admin que necesita el test 04; (3) la API arrancaba con `NODE_ENV=production` y el rate-limit (3/h) de `/auth/register` devolvía 429 a los tests; (4) con `NODE_ENV=test`, `main.ts` montaba Swagger y `patchNestJsSwagger` de nestjs-zod petaba el arranque; (5) selectores desfasados (`CardTitle` es `<div>` no heading; navegación admin por `page.goto` perdía la sesión en memoria; aserción del popup de impersonación demasiado estricta). Además: arranque de servidores con logs + **esperas fail-fast**, `retries: 2` + `forbidOnly` en CI. **`main` protegida** (branch protection): solo avanza vía PR con los **4 checks required** verdes (`Smoke E2E (Playwright)`, `Lint, typecheck & build`, `Tests database (Vitest)`, `Tests API e2e (Jest + Supertest)`) + 0 approvals + `enforce_admins`. De paso se arregló `ci.yml`, que estaba rojo de antes (build web sin `NEXT_PUBLIC_API_URL`, `test-database` sin seed, `test-api-e2e` con arg `-- --runInBand` sobrante + sin compilar `@storageos/database` + sin MinIO real). Flujo documentado en `README.md` → "Flujo de contribución". Pendiente: más cobertura e2e de los flujos de dinero.
