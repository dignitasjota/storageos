...
- ~~**Permisos más finos**~~ ✅ **Implementado** (capa de permisos discretos): catálogo `Permission` (`recurso:acción`) + `ROLE_PERMISSIONS` (mapa rol→permisos en código) + helpers en `packages/shared/src/auth/permissions.ts`. Backend: decorador `@RequirePermission(...)` + `PermissionsGuard` (APP_GUARD, complementario a `@Roles`); `/auth/me` devuelve los permisos efectivos del usuario. Primera regla más fina real: `invoices:refund` solo `owner` (aunque `manager` pueda emitir); el controller de invoices migrado a permisos (`invoices:read/write/manage/refund`). Frontend: hook `useHasPermission` + gating del botón "Reembolsar". Tests: unit `permissions.guard` 11/11 + e2e `permissions` 4/4. (Pendiente: 2FA obligatorio configurable por más roles; roles personalizados por tenant en BD.)
