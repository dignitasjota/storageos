...
- **WhatsApp Business API real** para dunning y avisos: ✅ provider `MetaWabaProvider` (Meta Cloud API, texto + plantilla) + selección por `WHATSAPP_PROVIDER=stub|meta_waba` + tests (`meta-waba-provider.spec` 4/4) + guía en `DEPLOYMENT.md §15`. **Pendiente (follow-up):** mapear el nombre de la plantilla WABA aprobada en `message_templates` para que el dunning proactivo la use (hoy el outbox envía el cuerpo renderizado como texto, válido solo dentro de la ventana de 24h). Bloqueante real: alta + aprobación de plantillas en Meta (operativo).
