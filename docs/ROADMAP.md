...
- ~~**Permisos más finos**~~ ✅ **Implementado** (capa de permisos discretos): catálogo `Permission` (`recurso:acción`) + `ROLE_PERMISSIONS` (mapa rol→permisos en código) + helpers en `packages/shared/src/auth/permissions.ts`. Backend: decorador `@RequirePermission(...)` + `PermissionsGuard` (APP_GUARD, complementario a `@Roles`); `/auth/me` devuelve los permisos efectivos del usuario. Primera regla más fina real: `invoices:refund` solo `owner` (aunque `manager` pueda emitir); el controller de invoices migrado a permisos (`invoices:read/write/manage/refund`). Frontend: hook `useHasPermission` + gating del botón "Reembolsar". Tests: unit `permissions.guard` 11/11 + e2e `permissions` 4/4.
- ~~**Roles personalizados por tenant (RBAC en BD)**~~ ✅ **Implementado** (v1, capa de roles custom): tabla `tenant_roles` `(tenant_id, name, description, permissions[], base_role)` con RLS + `users.tenant_role_id` (FK, ON DELETE SET NULL). Permisos efectivos = los del rol custom si está asignado, si no los del rol enum; se resuelven al emitir el access JWT (claim `permissions`) y en `/auth/me`. `PermissionsGuard` lee los permisos del token (fallback al mapa enum para tokens antiguos). CRUD `TenantRolesController` (`/settings/roles`, owner-only) + asignación `PATCH /settings/users/:id/tenant-role`. UI: página `/settings/roles` con matriz de permisos por recurso + selector de rol custom en el dialog de edición de usuario. Tests: e2e `tenant-roles` 3/3.
- ~~**RBAC v2 — migración completa de `@Roles` a `@RequirePermission`**~~ ✅ **Completada** (PRs #8–#12, fiel al catálogo): **ya no queda ningún `@Roles` en controllers de tenant**; toda la autorización la decide `@RequirePermission` + `ROLE_PERMISSIONS`, por lo que los roles personalizados controlan la app entera. Backend en 4 PRs — #8 operativa diaria (customers, contracts, reservations, leads, tasks, incidents…), #9 inventario/catálogo/comms (facilities, units, products, communications, templates, automations), #10 billing/pagos/accesos (payments, redsys, series, AEAT, holded, access), #11 admin/settings (users, invitations, tenant-roles, integrations, rgpd, imports, reports). Cambios intencionados por catálogo: `staff` gana escrituras que ya tenía concedidas (units, cobros `payments:charge`, `reports:run`, reintento de comms); `manager` pierde lo excluido (`customers:delete`, `billing:configure` de series, `users:manage`, `integrations:manage`, `rgpd:manage`). Nuevos permisos `leads/tasks/incidents:manage` + `imports:manage`. **#12 — gating de UI** (`useHasPermission`): pestañas de `/settings` (roles/security/billing/verifactu/integrations), acciones de `/settings/users`, botón Reembolsar y borrar cliente. Tests: e2e `permissions` 18/18. **Cleanup pendiente (menor):** retirar el `RolesGuard` global + decorador `@Roles`, ya inertes. (Pendiente aparte: 2FA obligatorio configurable por más roles.)
