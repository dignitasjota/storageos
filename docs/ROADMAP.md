...
- ~~**Playwright en CI bloqueante**~~ ✅ **Implementado**: el workflow `e2e.yml` (5 smoke tests) ahora es un gate fiable: siembra el super admin que necesitaba el test 04 (antes rojo permanente), arranca API+web con captura de logs y **esperas fail-fast** (exit 1 si un servicio no levanta), `retries: 2` + `forbidOnly` en CI, y vuelca los logs de servidor en fallo. Falta solo el toggle de **branch protection** en GitHub (marcar el check "E2E Playwright / Smoke E2E (Playwright)" como _required_ en Settings → Branches), que es ajuste de repo, no de código. Pendiente: más cobertura e2e de los flujos de dinero.
