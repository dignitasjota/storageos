...
**El MVP está cerrado, hardenizado, y emite facturas conformes a AEAT.** Tras Fase 10 ya no hay fase técnica obligatoria pendiente para vender en España. La siguiente prioridad la define el usuario: foco en **despliegue VPS + onboarding del primer cliente real** (alta dominio, configurar Resend/Stripe live, subir cert FNMT del cliente, cambiar `AEAT_MODE=sandbox` para dry-run, validar con factura de prueba). Backlog post-MVP en `docs/ROADMAP.md` (WhatsApp Business, API pública, app móvil PWA, multi-idioma, contables Holded/A3, smoke tests Playwright). Se prioriza cuando haya feedback de cliente.
