...
- **`/settings/integrations`** (Fase 14A.3): nueva página con tabs "API keys" + "Webhooks". API keys: CRUD + dialog post-create que muestra `sk_live_<tenantId>.<secret>` raw una sola vez (revealed-once) + lista con `prefix` + `last_used_at` + revoke. Webhooks: CRUD con URL + selector multi-evento (`invoice.paid/issued/overdue`, `contract.signed`, `lead.created`) + dialog post-create que muestra el `secret` raw una sola vez + tabla últimos 50 deliveries con expand para ver `payload` JSONB + `error_message`. Item en sidebar con icono `Plug`.
