...
**El MVP está cerrado, hardenizado, emite facturas F1/F2 conformes a AEAT, soporta rectificativas por diferencias o sustitución, fuerza 2FA al owner/manager, registra todas las acciones del super admin, separa el worker BullMQ del API en producción via `ENABLE_WORKERS_IN_API=false`, expone una spec OpenAPI versionada en `/v1/`, corre smoke tests Playwright en CI y permite a integradores externos consumir eventos vía API keys + webhooks HMAC.** Tras Fase 14 ya no hay fase técnica obligatoria pendiente para vender en España. La siguiente prioridad la define el usuario: foco en **despliegue VPS + onboarding del primer cliente real** (alta dominio, configurar Resend/Stripe live, subir cert FNMT del cliente, cambiar `AEAT_MODE=sandbox` para dry-run, validar con factura de prueba + una rectificativa + un webhook real a un Zapier de prueba). Tareas pendientes opcionales post-Fase 14: resolver deps DI faltantes en `WorkerModule` para activar los tests del worker (hoy `describe.skip` con `ioredis-mock` listo), monitorizar el endpoint `/api/csp-report` con alerta Grafana (CSP ya en enforce), enforce de scopes en API keys cuando haya un primer integrador real, añadir endpoints públicos `/v1/integrations/*` consumibles via `ApiKeyGuard`, añadir polling `getStatus` AEAT si AEAT introduce flow asíncrono. Backlog post-MVP en `docs/ROADMAP.md` (WhatsApp Business, app móvil PWA, multi-idioma, contables Holded/A3).
